...
public class ArmorPackModule extends PackModule<ArmorPackModule> {
    ...
    private void postProcess(@NotNull PackPostProcessContext<ArmorPackModule> context) {
        ...
        for (ArmorItem armorItem : armorItems) {
            ...
            ResourceLocation armorTextureLocation = armorItem.components().get(DataComponents.EQUIPPABLE).assetId().get().location(); // TODO See if this is correct
            ...
            description.textures(new HashMap<>() {
                {
                    put("default", String.format(BEDROCK_ARMOR_TEXTURE_LOCATION, context.mod().id(), armorTextureLocation.getPath(), (armorItem.components().get(DataComponents.EQUIPPABLE).slot() == EquipmentSlot.LEGS ? 2 : 1)));
